# Second experiment: we use row context
import csv
import requests
import pandas as pd
import time
...
def annotate_cells():
    ...
    df_cea_targets = pd.read_csv(
        "Dataset/Valid/targets/cea_targets.csv", header=None)
    ...
    df_annotated[3] = df_annotated.apply(
        lambda row: get_wikidata_entity(row[0], row[1], row[2]), axis=1)

    # Save the annotated df
    df_annotated.to_csv(
        'Dataset/output/cea_wikidata_second_experiment.csv', index=False, header=False)
