...
def get_annotations_for_column(table_name, column_index):
    df_annotation_retrieved = pd.read_csv(
        f"DataSets/Valid/cea annotation/output_with_row_context.csv", header=None)
...
def annotate_relationships():
    ...
    df_annotated.to_csv(
        'DataSets/Valid/cpa annotation/output_with_row_context.csv', index=False, header=False)
