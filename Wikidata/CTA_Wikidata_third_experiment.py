...
def annotate_column_cells(table_name, column_index):
    result_column = []
    df_cea = pd.read_csv(
        f"Dataset/output/cea annotation/cea_wikidata_third_experiment.csv")
...
def annotate_columns():
    ...
    df_annotated.to_csv(
        'Dataset/output/cta annotation/cta_wikidata_third_experiment.csv', index=False, header=False)
